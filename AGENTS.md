...
- Presentation (`UsageNotificationService`) is the stateful/threaded half: it queues toasts and shows them one at a time. It respects Do Not Disturb / full-screen via `SHQueryUserNotificationState` — while suppressed it holds only the **latest** alert and surfaces that single toast when the user is available again (older alerts during the block are stale for a usage gauge, so they are dropped, not replayed). It does **not** annotate the toast with a "N missed" count — that count was removed as meta-noise that only confused users. A query failure is treated as "present anyway", never as permanent silence.
- The toast is a **fixed-size window** (`WidthDip`/`HeightDip` in `NotificationWindow`; currently 360×76 DIP, the standard Windows toast width) with a left icon column and a wrapping title + message. Because it does not auto-size, keep localized title/message templates concise: a title long enough to wrap past two lines clips. The English threshold title was the tightest case and drove the 320→360 widen.
...
- Developer visual-QA switches (read from the real process command line, since unpackaged WinUI drops EXE args), each running beside the normal tray instance via a separate single-instance key: `--notification-demo` cycles every alert kind in light and dark; `--notification-longest` shows the worst-case longest title+message in Korean/English/Japanese (built straight from the resource table) to confirm the fixed-size window never clips.
